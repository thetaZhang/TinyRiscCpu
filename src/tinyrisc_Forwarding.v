...
`define EX_HZD_1 (if_id_pc_sel_in == `PC_PLUS4) && \
                 (id_ex_reg_we_in) && \
                 (id_ex_rd_in != 0) && \
                 (if_id_reg_use_in) && \
                 ((id_ex_rd_in == if_id_rs1_addr_in))
`define EX_HZD_2 (if_id_pc_sel_in == `PC_PLUS4) && \
                 (id_ex_reg_we_in) && \
                 (id_ex_rd_in != 0) && \
                 (if_id_reg_use_in) && \
                 ((id_ex_rd_in == if_id_rs2_addr_in))

`define MEM_HZD_1 (if_id_pc_sel_in == `PC_PLUS4) && \
                  (ex_mem_reg_we_in) && \
                  (ex_mem_rd_in != 0) && \
                  (!(`EX_HZD_1)) && \
                  (if_id_reg_use_in) && \
                  ((ex_mem_rd_in == if_id_rs1_addr_in))
`define MEM_HZD_2 (if_id_pc_sel_in == `PC_PLUS4) && \
                  (ex_mem_reg_we_in) && \
                  (ex_mem_rd_in != 0) && \
                  (!(`EX_HZD_2)) && \
                  (if_id_reg_use_in) && \
...
module tinyrisc_Forwarding(
    ...
    input [`REG_ADDR_WIDTH - 1 : 0]   id_ex_rd_in,
